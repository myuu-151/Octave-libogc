...
Octave-libogc is a fork of the [Octave 3D Game Engine](https://github.com/mholtkamp/octave) that focuses exclusively on its GameCube platform

> **Why this fork?**
> - Stays on the original **libogc** — upstream Octave now requires libogc2 and a third-party package repository.
> - A stock devkitPro install (`gamecube-dev` / `wii-dev`) is all you need for GameCube and Wii.
> - No Docker required — everything builds natively on Windows and Linux.
...
2. Install Wii/3DS development libraries (Optional) (<https://devkitpro.org/wiki/Getting_Started>)
   ...
   - Restart computer
3. If you want to package for GameCube, install the standard devkitPro GameCube libraries (libogc):
